...
def calc_lambda_return(
    rewards, values, termination, gamma, lam, dtype=torch.float32, device=None
):
    ...
    def update(
        ...
        with torch.autocast(
            ...
            lambda_return = calc_lambda_return(
                reward, value, termination, self.gamma, self.lambd, device=self.device
            )
            ...
            norm_ratio = torch.max(
                torch.ones(1, device=self.device), S
