...
class EncoderBN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, "B L C H W -> (B L) C H W").contiguous()
        x = self.backbone(x)
        x = rearrange(x, "(B L) C H W -> B L (C H W)", B=batch_size).contiguous()
        return x
...
class DecoderBN(nn.Module):
    ...
    def forward(self, sample):
        ...
        obs_hat = rearrange(
            obs_hat, "(B L) C H W -> B L C H W", B=batch_size
        ).contiguous()
        return obs_hat
...
class DistHead(nn.Module):
    ...
    def forward_post(self, x):
        ...
        logits = rearrange(
            logits, "B L (K C) -> B L K C", K=self.stoch_dim
        ).contiguous()
...
class WorldModel(nn.Module):
    def __init__(
        ...
        # Create GradScaler conditionally based on device
    ...
    def flatten_sample(self, sample):
        return rearrange(sample, "B L K C -> B L (K C)").contiguous()
    ...
    def init_imagine_buffer(self, imagine_batch_size, imagine_batch_length, dtype):
        ...
        if (
            ...
            self.latent_buffer = torch.zeros(
                latent_size, dtype=dtype, device=self.device
            )
            self.hidden_buffer = torch.zeros(
                hidden_size, dtype=dtype, device=self.device
            )
            self.action_buffer = torch.zeros(
                scalar_size, dtype=dtype, device=self.device
            )
