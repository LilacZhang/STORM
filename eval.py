...
def build_vec_env(env_name, image_size, num_envs, seed=0):
    ...
    def lambda_generator(env_name, image_size, env_seed):
        return lambda: build_single_env(env_name, image_size, env_seed)

    env_fns = []
    env_fns = [
        lambda_generator(env_name, image_size, seed + i) for i in range(num_envs)
    ]
...
def eval_episodes(
    ...
    world_model.eval()
    agent.eval()
    vec_env = build_vec_env(env_name, image_size, num_envs=num_envs, seed=0)
    print(
        "Current env: "
...
if __name__ == "__main__":
    ...
    # set seed
    seed_np_torch(seed=conf.BasicSettings.Seed, device=device)
    ...
    world_model = train.build_world_model(conf, action_dim, device)
