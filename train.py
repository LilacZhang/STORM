...
from utils import seed_np_torch, Logger, load_config, DeviceManager
...
def build_single_env(env_name, image_size, seed):
    """Build a single environment with auto-detection for Atari/MuJoCo"""
    return env_wrapper.build_single_env(env_name, image_size, seed)
...
def joint_train_world_model_agent(
    ...
    for total_steps in tqdm(range(max_steps // num_envs)):
        ...
        # Use life_loss if available (Atari), otherwise just use done
        # In vectorized envs, info might be a dict of arrays or a dict
        if isinstance(info, dict) and "life_loss" in info:
            life_loss = info["life_loss"]
        else:
            life_loss = done  # For MuJoCo or other envs without life_loss
        replay_buffer.append(
            current_obs, action, reward, np.logical_or(done, life_loss)
        )
        ...
        if done_flag.any():
            for i in range(num_envs):
                if done_flag[i]:
                    ...
                    # Log episode steps - frame_number for Atari, fallback for others
                    if "episode_frame_number" in current_info:
                        episode_steps = current_info["episode_frame_number"][i] // 4
                    else:
                        episode_steps = 0  # MuJoCo doesn't track frame numbers
                    logger.log(f"sample/{env_name}_episode_steps", episode_steps)
...
if __name__ == "__main__":
    ...
    device_manager = DeviceManager(
        device_config=conf.BasicSettings.Device, use_amp=conf.BasicSettings.UseAMP
    )
    device = device_manager.get_device()

    # set seed
    seed_np_torch(seed=args.seed, device=device)
    # tensorboard writer
    ...
    if conf.Task == "JointTrainAgent":
        ...
        # Get action dimension - works for both Discrete and discretized Box spaces
        if hasattr(dummy_env.action_space, "n"):
            action_dim = dummy_env.action_space.n
        else:
            raise ValueError(
                f"Unsupported action space type: {type(dummy_env.action_space)}"
            )
